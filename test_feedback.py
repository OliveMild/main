...
class TestFeedback(unittest.TestCase):
    ...
    def test_submit_feedback_empty_user(self):
        with self.assertRaises(ValueError):
            feedback.submit_feedback("", "Message", 3)

    def test_submit_feedback_whitespace_user(self):
        with self.assertRaises(ValueError):
            feedback.submit_feedback("   ", "Message", 3)

    def test_submit_feedback_empty_message(self):
        with self.assertRaises(ValueError):
            feedback.submit_feedback("Alice", "", 3)

    def test_submit_feedback_whitespace_message(self):
        with self.assertRaises(ValueError):
            feedback.submit_feedback("Alice", "   ", 3)

    def test_load_feedback_corrupted_file(self):
