...
def _validate_email(email: str) -> None:
    ...
    # Require exactly one '@', a non-empty local part, a non-empty domain with
    # at least one dot, and no whitespace anywhere.
    pattern = r"^[^@\s]+@[^@\s]+\.[^@\s.][^@\s]*$"
    if not re.match(pattern, email) or email.count("@") != 1:
