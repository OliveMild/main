...
def _validate_username(username: str) -> None:
    ...
    if len(username) < _USERNAME_MIN_LENGTH or len(username) > _USERNAME_MAX_LENGTH:
        raise ValidationError(
            f"Username must be between {_USERNAME_MIN_LENGTH} and {_USERNAME_MAX_LENGTH} characters."
        )
...
def _validate_email(email: str) -> None:
    ...
    # Local part must be non-empty, free of whitespace, must not start or end
    # with a dot, and must not contain consecutive dots.
    if (
        not local
        or re.search(r"\s", local)
        or local.startswith(".")
        or local.endswith(".")
        or ".." in local
    ):
