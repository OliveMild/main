...
class TestFeedModule(unittest.TestCase):
    ...
    def _capture_display_feed(self):
        """Call display_feed() and return all printed output as a single string."""
        with patch("builtins.print") as mock_print:
            feed.display_feed()
        return " ".join(str(c) for c in mock_print.call_args_list)

    def test_display_empty_feed(self):
        output = self._capture_display_feed()
        self.assertIn("The feed is empty.", output)
