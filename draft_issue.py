...
def list_draft_issues(owner, repo, token=None):
    ...
    Returns:
        List of issue dicts.  At most 100 issues are returned (GitHub's
        maximum per-page limit).  If you expect more than 100 draft issues,
        use the GitHub API directly with pagination.
    """
    token = token or os.environ.get("GITHUB_TOKEN")
    if not token:
        raise ValueError("A GitHub token is required (pass token= or set GITHUB_TOKEN).")
