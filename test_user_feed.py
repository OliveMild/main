...
class TestExceptionHierarchy(unittest.TestCase):
    ...
    def test_invalid_email_is_user_feed_error(self):
        self.assertTrue(issubclass(InvalidEmailError, UserFeedError))
...
class TestFeedItemConstruction(unittest.TestCase):
    ...
    def test_valid_email_is_stored(self):
        item = FeedItem(user_id="alice", title="Post", email="alice@example.com")
        self.assertEqual(item.email, "alice@example.com")

    def test_default_email_is_empty_string(self):
        item = FeedItem(user_id="alice", title="Post")
...
class TestFeedItemValidationErrors(unittest.TestCase):
    ...
    def test_invalid_email_no_domain_raises(self):
        with self.assertRaises(InvalidEmailError):
            FeedItem(user_id="alice", title="Post", email="user@")

    def test_invalid_email_no_tld_raises(self):
        with self.assertRaises(InvalidEmailError):
            FeedItem(user_id="alice", title="Post", email="user@nodot")

    def test_invalid_email_none_raises(self):
        with self.assertRaises(InvalidEmailError):
            FeedItem(user_id="alice", title="Post", email=None)

    def test_email_too_long_raises(self):
        long_local = "a" * (EMAIL_MAX_LENGTH - len("@example.com") + 1)
        with self.assertRaises(InvalidEmailError):
            FeedItem(user_id="alice", title="Post", email=f"{long_local}@example.com")

    def test_valid_email_subdomain(self):
        item = FeedItem(user_id="alice", title="Post", email="user@mail.example.com")
        self.assertEqual(item.email, "user@mail.example.com")

    def test_empty_email_is_allowed(self):
        item = FeedItem(user_id="alice", title="Post", email="")
        self.assertEqual(item.email, "")
...
class TestFeedItemSetters(unittest.TestCase):
    ...
    def test_set_valid_email(self):
        self.item.email = "alice@example.com"
        self.assertEqual(self.item.email, "alice@example.com")

    def test_set_invalid_email_raises(self):
        with self.assertRaises(InvalidEmailError):
            self.item.email = "not-an-email"

    def test_set_email_to_empty_string(self):
        self.item.email = "alice@example.com"
        self.item.email = ""
        self.assertEqual(self.item.email, "")
