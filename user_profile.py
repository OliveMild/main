...
def _validate_username(username: str) -> None:
    ...
    if len(username) > _USERNAME_MAX:
        raise ValidationError(
            f"Username must not exceed {_USERNAME_MAX} characters."
        )
    if username != username.strip():
        raise ValidationError("Username must not have leading or trailing whitespace.")
...
class UserProfile:
    ...
    def update(
        ...
        new_avatar_url = (
            avatar_url if avatar_url is not None else self.avatar_url
        )
