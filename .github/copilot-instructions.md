...
- **Language**: Python 3
- **Package manager**: pip (standard library only; no `requirements.txt` at this time)
...
There is no dedicated test suite at this time. Manually verify `hello.py` runs without errors:
...
- Branch names: `<type>/<short-description>`, e.g. `feat/add-greeting`, `fix/typo-in-readme`, `docs/update-instructions`.
- Commit messages: use the imperative mood, e.g. `Add greeting function`, `Fix off-by-one error`.
...
- **Secrets**: Do not commit secrets or credentials. Use GitHub Actions secrets (Settings → Secrets and variables → Actions) if CI ever needs sensitive values.
