...
class TestValidateEmail(unittest.TestCase):
    ...
    def test_invalid_email_consecutive_dots_in_domain(self):
        self.assertFalse(validate_email('user@example..com'))

    def test_invalid_email_leading_dot_in_local(self):
        self.assertFalse(validate_email('.user@example.com'))

    def test_invalid_email_trailing_dot_in_local(self):
        self.assertFalse(validate_email('user.@example.com'))

    def test_invalid_email_consecutive_dots_in_local(self):
        self.assertFalse(validate_email('us..er@example.com'))

    def test_invalid_email_exceeds_max_length(self):
        local = 'a' * 244
        self.assertFalse(validate_email(f'{local}@example.com'))
...
class TestRegisterUser(unittest.TestCase):
    ...
    def test_leading_dot_email_returns_error(self):
        result = register_user('eve', '.eve@example.com', 'securepass')
        self.assertFalse(result['success'])
        self.assertIn('Invalid email', result['message'])

    def test_trailing_dot_email_returns_error(self):
        result = register_user('eve', 'eve.@example.com', 'securepass')
        self.assertFalse(result['success'])
        self.assertIn('Invalid email', result['message'])
