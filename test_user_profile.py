...
def test_invalid_email_missing_at_raises():
    with pytest.raises(UserProfileError, match="Invalid email address"):
        UserProfile(username="Alice_99", email="notanemail")


def test_invalid_email_missing_domain_raises():
    with pytest.raises(UserProfileError, match="Invalid email address"):
        UserProfile(username="Alice_99", email="user@")


def test_invalid_email_missing_local_raises():
    with pytest.raises(UserProfileError, match="Invalid email address"):
        UserProfile(username="Alice_99", email="@example.com")


def test_invalid_email_no_tld_raises():
    with pytest.raises(UserProfileError, match="Invalid email address"):
        UserProfile(username="Alice_99", email="user@nodot")


def test_invalid_email_leading_dot_in_domain_raises():
    with pytest.raises(UserProfileError, match="Invalid email address"):
        UserProfile(username="Alice_99", email="user@.domain.com")
