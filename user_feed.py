...
class InvalidEmailError(UserFeedError):
...
def _validate_email(email):
    ...
    if len(email) > EMAIL_MAX_LENGTH:
        raise InvalidEmailError(
            f"email must not exceed {EMAIL_MAX_LENGTH} characters."
        )
...
class FeedItem:
    ...
    def __init__(self, user_id, title, content="", email=""):
...
class FeedStore:
    ...
    def add_item(self, user_id, title, content="", email=""):
        """Create and store a new feed item; returns the item."""
        item = FeedItem(user_id=user_id, title=title, content=content, email=email)
